 ...
 * `{region}-{project}.cloudfunctions.net/{functionName}` HTTPS endpoint.
 * We use httpsCallableFromURL so the JS SDK calls the correct endpoint
 * regardless of the region configured in src/config/firebase.js.
 *
 * URL was added in feature/deduccion-y-migracion-3.5 — needs to be
 * deployed to southamerica-east1 before the frontend can call it.
 */

const REGION = 'southamerica-east1';
const PROJECT_ID = 'caldero-envio';
const SPEND_CALDERO_URL = `https://${REGION}-${PROJECT_ID}.cloudfunctions.net/spendCaldero`;
